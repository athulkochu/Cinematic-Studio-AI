      ...
      <Layout>
        <div className="flex-1 flex flex-col items-center justify-center gap-4">
          <p className="text-muted-foreground">
            Project not found or error loading.
          </p>
  ...
  const handleExport = () => {
    exportVideo.mutate(
      { id: projectId },
      {
        onSuccess: () => {
          toast.success(
            "Stitching frames into video — this can take a minute…",
          );
          queryClient.invalidateQueries({
            queryKey: getGetProjectQueryKey(projectId),
          });
        ...
        <header className="flex-shrink-0 border-b border-border bg-card/30 backdrop-blur-md px-6 py-4 flex items-center justify-between z-10 relative">
          ...
          <div className="flex items-center gap-3">
            <ScheduleDialog
              ...
              disabled={
                project.status !== "rendered" && project.status !== "published"
              }
            />
            ...
            <Button
              ...
              disabled={
                isExporting || !hasRenderedFrames || exportVideo.isPending
              }
              className={cn(
              ...
              <div className="flex items-center justify-between mb-4">
                <TabsList className="bg-card/50 border border-border/50">
                  ...
                  <TabsTrigger value="characters" className="gap-2">
                    <Users size={14} /> Cast ({project.characters.length})
                  </TabsTrigger>
                </TabsList>
              </div>
              <p className="text-xs text-muted-foreground mb-6">
                This studio generates a story plan, character sheets, and one
                still frame per scene — not a finished video file. Connect a
                video model (Runway, Pika, etc.) to extend frames into motion.
              </p>
                ...
                {(videoUrl || isExporting) && (
                  <div className="mb-8 max-w-4xl">
                    ...
                    <p className="text-[11px] text-muted-foreground/70 mt-2">
                      Storyboard frames stitched at each scene's duration. No
                      motion or audio yet — those come in a later phase.
                    </p>
                  </div>
                )}
                  ...
                  <div className="flex gap-4 overflow-x-auto pb-4 snap-x snap-mandatory hide-scrollbar">
                    {project.scenes
                      ...
                      .map((scene, idx) => (
                        <div
                          key={scene.id}
                          className="snap-start flex-shrink-0 w-64 bg-card rounded-lg border border-border overflow-hidden group"
                        >
          ...
          <aside className="w-full md:w-80 border-l border-border bg-card/20 flex-shrink-0 flex flex-col hidden md:flex">
            <div className="p-5 border-b border-border">
              <h2 className="font-semibold flex items-center gap-2">
                <Settings2 size={16} className="text-primary" /> Continuity
                Engine
              </h2>
              <p className="text-xs text-muted-foreground mt-1">
                Global variables enforced across all scene prompts.
              </p>
            </div>
...
function CharacterCard({ character }: { character: any }) {
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      if (
        face !== lastSaved.current.face ||
        clothing !== lastSaved.current.clothing
      ) {
        updateCharacter.mutate(
          ...
          },
          {
            onSuccess: () => {
              queryClient.invalidateQueries({
                queryKey: getGetProjectQueryKey(character.projectId),
              });
      ...
      <div className="aspect-square bg-muted relative group">
        ...
        <div className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center backdrop-blur-sm">
          <Button
            ...
            disabled={regenerateRef.isPending}
            className="gap-2"
          >
            <RefreshCw
              size={14}
              className={regenerateRef.isPending ? "animate-spin" : ""}
            />
            Regen Reference
          </Button>
        </div>
...
function SceneEditor({ scene, characters }: { scene: any; characters: any[] }) {
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      if (
        prompt !== lastSaved.current.prompt ||
        action !== lastSaved.current.action
      ) {
        updateScene.mutate(
          {
            id: scene.id,
          ...
          },
          {
            onSuccess: () => {
              queryClient.invalidateQueries({
                queryKey: getGetProjectQueryKey(scene.projectId),
              });
      ...
      <div className="w-full md:w-64 bg-muted/30 flex-shrink-0 flex flex-col relative border-r border-border/50">
        ...
        <div className="p-4 flex-1 flex flex-col">
          ...
          >
            <RefreshCw
              size={12}
              className={regenScene.isPending ? "animate-spin" : ""}
            />
      ...
      <div className="flex-1 p-5 flex flex-col">
        ...
        {scene.previousSummary && (
          <div className="mb-4 p-3 bg-secondary/20 rounded border border-border/50 text-xs text-muted-foreground italic border-l-2 border-l-primary/50">
            <span className="font-semibold block mb-1 not-italic text-[10px] uppercase text-primary">
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <Input
                      placeholder="#cinematic #aivideo #shortfilm"
                      {...field}
                    />
                  </FormControl>
                  <FormMessage />
